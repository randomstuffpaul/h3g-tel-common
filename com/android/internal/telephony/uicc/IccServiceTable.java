...
public abstract class IccServiceTable {
    ...
    protected boolean isSSTAvailable(int service) {
        boolean z = true;
        int offset = service / 4;
        if (offset >= this.mServiceTable.length) {
            Rlog.e(getTag(), "isSSTAvailable for service " + (service + 1) + " fails, max service is " + (this.mServiceTable.length * 8));
            return false;
        }
        if ((this.mServiceTable[offset] & (1 << ((service % 4) * 2))) == 0) {
            z = false;
        }
        return z;
    }

    protected boolean isSSTActive(int service) {
        boolean z = true;
        int offset = service / 4;
        if (offset >= this.mServiceTable.length) {
            Rlog.e(getTag(), "isActive for service " + (service + 1) + " fails, max service is " + (this.mServiceTable.length * 4));
            return false;
        }
        if ((this.mServiceTable[offset] & (1 << (((service % 4) * 2) + 1))) == 0) {
            z = false;
        }
        return z;
    }
